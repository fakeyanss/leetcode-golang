...
// 单调栈，占元素存储price和第几天idx
// 每次计算，pop栈顶元素直到栈顶元素比当前price大，此时结果为
// 当前idx-栈顶元素idx
func (ss *StockSpanner) Next(price int) int {
    ss.idx++
    for price >= ss.stack[len(ss.stack)-1][0] {
        ss.stack = ss.stack[:len(ss.stack)-1]
    }
    ss.stack = append(ss.stack, [2]int{price, ss.idx})
    return ss.idx - ss.stack[len(ss.stack)-2][1]
}
