...
func maxProfit(prices []int, strategy []int, k int) int64 {
	...
	for i := 0; i < n; i++ {
		...
		priceSum[i+1] = int64(priceSum[i]) + int64(prices[i])
	}
	// 修改策略后，利润分为3部分，假设修改策略的区间为[i-k+1,i]，即i为右侧端点
	// 1. 左侧[0,i-k]的profitSum
	// 2. 中间的[i-k/2+1,i]的prices[i]之和
	// 3. 右侧[i+1,n-1]的profitSum
	res := profitSum[n]
	for i := k - 1; i < n; i++ {
