...
func subtreeWithAllDeepest(root *TreeNode) *TreeNode {
	// dfs返回当前树节点的最大深度，以及最深节点的最近公共祖先LCA
	var dfs func(*TreeNode) (int, *TreeNode)
	dfs = func(node *TreeNode) (depth int, lca *TreeNode) {
		if node == nil {
			return 0, nil
		}
		leftDepth, leftLCA := dfs(node.Left)
		rightDepth, rightLCA := dfs(node.Right)
		if leftDepth > rightDepth { // 左子树深度更大，LCA在左子树中
			return leftDepth + 1, leftLCA
		}
		if leftDepth < rightDepth { //右子树深度更大，LCA在右子树中
			return rightDepth + 1, rightLCA
		}
		return leftDepth + 1, node // 左右子树深度一样，LCA就是node
	}
	_, lca := dfs(root)
	return lca
}
