...
func deleteNode450(root *TreeNode, key int) *TreeNode {
	...
	if root.Val == key {
		...
		// 左右子树都有节点，则将右子树最小值替换此节点
		rightMin := findRightMinInBST450(root.Right)
		// 删除rightMin节点
		root.Right = deleteNode450(root.Right, rightMin.Val)
		// 再替换rightMin与root
...
func findRightMinInBST450(root *TreeNode) *TreeNode {
	for root.Left != nil {
		root = root.Left
	}
	return root
